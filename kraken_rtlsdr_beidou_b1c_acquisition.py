#!/usr/bin/env python3
"""
Real-time BeiDou B1C (pilot) Signal Acquisition using KrakenSDR/RTL-SDR
- Reception: RTL-SDR/KrakenSDR (8-bit samples)
- Acquisition: BeiDou B1C (primary) using FFT-based search

Mirrors the Galileo flow in kraken_rtlsdr_galileo_e1c_acquisition.py.
"""
...
class KrakenRTLSDRBeiDouB1C:
    def __init__(self,
        ...
        self.acq = BeiDouB1CAcquisition(sampling_freq=sample_rate, IF=self.if_hz, acq_threshold=acq_threshold)

        self.sdr = None
        self.receiving = False
        self.receive_thread = None

        # For B1C primary acquisition, need ~ 12 ms for margin (10 ms code)
        ...
        self.min_signal_power = 5e-6

        signal.signal(signal.SIGINT, self._signal_handler)

    def _signal_handler(self, signum, frame):
        print("\n🛑 Stopping BeiDou B1C acquisition...")
        self.stop_reception()
        sys.exit(0)
    ...
    def start_reception(self):
        if not self.setup_rtlsdr():
            return False
        print("🚀 Starting BeiDou B1C signal reception...")
    ...
    def _process_once(self):
        ...
        try:
            ...
            print(f"❌ B1C processing error: {e}")
            return [], [], 0.0
...
def main():
    print("🛰️ Real-time BeiDou B1C Signal Acquisition")
...
if __name__ == "__main__":
    print("Usage:")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py                    # Continuous monitoring (default)")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py single             # Single acquisition")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py device=1           # Use RTL-SDR device #1")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py gain=40            # Set gain to 40 dB")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py rate=3.2           # 3.2 MHz sample rate")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py bandwidth=3.2      # 3.2 MHz tuner bandwidth")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py offset_khz=250     # +250 kHz RX LO offset (IF)")
    print("  python kraken_rtlsdr_beidou_b1c_acquisition.py threshold=1.8      # Lower threshold")
    print()
    sys.exit(main())
